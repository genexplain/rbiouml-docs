...
biouml.ls("analyses/Workflows/Common")
...
biouml.ls("analyses/Workflows/Common/Find master regulators in networks (GeneWays)")
...
biouml.workflow("analyses/Workflows/Common/Find master regulators in networks (GeneWays)",
                list("Input gene set"=input,Species="Human (Homo sapiens)","Results folder"=output),
                T,T);

## Loading results into local data tables

annotated.genes <- biouml.get(paste(output,"/Regulator genes annot",sep=""))
regulators      <- biouml.get(paste(output,"/Regulators Upstream 4",sep=""))
